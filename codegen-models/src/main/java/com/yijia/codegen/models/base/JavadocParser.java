...
import com.yijia.codegen.models.base.javadoc.JavadocBlockTag;
...
class JavadocParser {
	...
	public static Javadoc parse(String commentContent) {
		...
		if (indexOfFirstBlockTag == -1) {
			...
			blockLines = BLOCK_PATTERN.splitAsStream(tagBlock).filter(Utils.STRING_NOT_EMPTY).map(s -> BLOCK_TAG_PREFIX + s.replace("\n", " ").trim()).collect(Collectors.toList());
		}
