...
import com.yijia.codegen.models.base.ast.body.CallableDeclaration;
...
import com.yijia.codegen.models.base.utils.Log;
...
public abstract class NodeGenerator extends Generator {
	...
	/**
	 * Utility method that looks for a method or constructor with an identical signature as "callable" and replaces it with callable. If not found, adds callable. When the new
	 * callable has no javadoc, any old javadoc will be kept.
	 */
	...
	/**
	 * Utility method that looks for a method or constructor with an identical signature as "callable" and replaces it with callable. If not found, fails. When the new callable has
	 * no javadoc, any old javadoc will be kept.
	 */
	protected void replaceWhenSameSignature(ClassOrInterfaceDeclaration containingClassOrInterface, CallableDeclaration<?> callable) {
		addMethod(containingClassOrInterface, callable, () -> {
			throw new AssertionError(
					f("Wanted to regenerate a method with signature %s in %s, but it wasn't there.", callable.getSignature(), containingClassOrInterface.getNameAsString()));
		});
	}

	private void addMethod(ClassOrInterfaceDeclaration containingClassOrInterface, CallableDeclaration<?> callable, Runnable onNoExistingMethod) {
		...
		if (existingCallables.size() > 1) {
			throw new AssertionError(
					f("Wanted to regenerate a method with signature %s in %s, but found more than one.", callable.getSignature(), containingClassOrInterface.getNameAsString()));
		}
