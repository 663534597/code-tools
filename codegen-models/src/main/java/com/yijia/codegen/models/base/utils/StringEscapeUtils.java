...
public class StringEscapeUtils {
	...
	private static final CharSequenceTranslator ESCAPE_JAVA = new AggregateTranslator(new LookupTranslator(new String[][] { { "\"", "\\\"" }, { "\\", "\\\\" }, }),
			new LookupTranslator(JAVA_CTRL_CHARS_ESCAPE.clone()));

	private static final CharSequenceTranslator UNESCAPE_JAVA = new AggregateTranslator(new OctalUnescaper(), new UnicodeUnescaper(),
			new LookupTranslator(JAVA_CTRL_CHARS_UNESCAPE.clone()), new LookupTranslator(new String[][] { { "\\\\", "\\" }, { "\\\"", "\"" }, { "\\'", "'" }, { "\\", "" } }));

	/**
	 * Adapted from apache commons-lang3 project.
	 * <p>
	 * An API for translating text. Its core use is to escape and unescape text. Because escaping and unescaping is completely contextual, the API does not present two separate
	 * signatures.
	 * @since 3.0
	 */
	private static abstract class CharSequenceTranslator {
		...
		/**
		 * Translate a set of codepoints, represented by an int index into a CharSequence, into another set of codepoints. The number of codepoints consumed must be returned, and
		 * the only IOExceptions thrown must be from interacting with the Writer so that the top level API may reliably ignore StringWriter IOExceptions.
	...
	private static class LookupTranslator extends CharSequenceTranslator {
		 ...
		 * <p>
		 * Note that, as of Lang 3.1, the key to the lookup table is converted to a java.lang.String. This is because we need the key to support hashCode and equals(Object),
