...
public interface NodeWithModifiers<N extends Node> {
	...
	default N addModifier(Modifier... modifiers) {
		...
		newModifiers.addAll(Arrays.stream(modifiers).collect(Collectors.toCollection(() -> EnumSet.noneOf(Modifier.class))));
	...
	default N removeModifier(Modifier... m) {
		EnumSet<Modifier> newModifiers = getModifiers().clone();
		newModifiers.removeAll(Arrays.stream(m).collect(Collectors.toCollection(() -> EnumSet.noneOf(Modifier.class))));
		setModifiers(newModifiers);
		return (N) this;
	}
