...
public interface NodeWithParameters<N extends Node> {
	...
	default boolean hasParametersOfType(String... paramTypes) {
		return getParameters().stream().map(p -> p.getType().toString()).collect(toSet()).equals(Stream.of(paramTypes).collect(toSet()));
	}
	...
	default boolean hasParametersOfType(Class<?>... paramTypes) {
		return getParameters().stream().map(p -> p.getType().toString()).collect(toSet()).equals(Stream.of(paramTypes).map(Class::getSimpleName).collect(toSet()));
	}
}
