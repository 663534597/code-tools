...
import com.yijia.codegen.models.base.utils.PositionUtils;
import java.util.Collections;
import java.util.LinkedList;
import java.util.List;
import java.util.TreeSet;
import java.util.stream.Collectors;
import static com.yijia.codegen.models.base.ast.Node.NODE_BY_BEGIN_POSITION;
...
class CommentsInserter {
	...
	void insertComments(Node node, TreeSet<Comment> commentsToAttribute) {
		...
		for (Node child : children) {
			...
			commentsInsideChild.addAll(commentsToAttribute.stream().filter(c -> c.getRange().isPresent())
					.filter(c -> PositionUtils.nodeContains(child, c, configuration.isDoNotConsiderAnnotationsAsNodeStartForCodeAttribution())).collect(Collectors.toList()));
	...
	private void attributeLineCommentsOnSameLine(TreeSet<Comment> commentsToAttribute, List<Node> children) {
		/*
		 * I can attribute in line comments to elements preceeding them, if there is something contained in their line
		 */
		List<Comment> attributedComments = new LinkedList<>();
		commentsToAttribute.stream().filter(comment -> comment.getRange().isPresent()).filter(Comment::isLineComment)
				.forEach(comment -> children.stream().filter(child -> child.getRange().isPresent()).forEach(child -> {
